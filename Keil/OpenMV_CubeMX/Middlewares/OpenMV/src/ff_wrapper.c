...
NORETURN static void ff_fail(FIL *fp, FRESULT res)
{
    ...
    DEBUG("mp_type_OSError, ffs_strerror(%d)));",res);
}
...
NORETURN static void ff_read_fail(FIL *fp)
{
    ...
    DEBUG("Failed to read requested bytes!");
}
...
NORETURN static void ff_write_fail(FIL *fp)
{
    ...
    DEBUG("Failed to write requested bytes!");
}
...
FRESULT f_opendir_helper(DIR *dp, const TCHAR *path) {
    FATFS *fs = lookup_path(&path);
    if (fs == NULL) {
        return FR_NO_PATH;
    }
