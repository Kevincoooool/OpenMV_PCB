...
//#define TIMHandle   
#define DMAHandle   hdma2_stream1
#define DCMIHandle  hdcmi

static volatile int line = 0;
extern uint8_t _line_buf[OMV_LINE_BUF_SIZE] = {0}; 
...
static int dcmi_config(uint32_t jpeg_mode)
{
   ...
   // Initialize the DCMI
    ...
    #endif 
    DCMI_Init(jpeg_mode,SENSOR_HW_FLAGS_GET(&sensor, SENSOR_HW_FLAGS_VSYNC) ,\
                        SENSOR_HW_FLAGS_GET(&sensor, SENSOR_HW_FLAGS_HSYNC) ,\
                        SENSOR_HW_FLAGS_GET(&sensor, SENSOR_HW_FLAGS_PIXCK));
...
int sensor_init()
{
    ...
    sensor.snapshot = sensor_snapshot_simple ;

    if (sensor.slv_addr == LEPTON_ID) {
        sensor.chip_id = LEPTON_ID;
        DEBUG(" Sensor is not supported.");
        return -3;
        #if 0
        ...
        if (sensor.chip_id == MT9V034_ID) {
            ...
            switch (sensor.chip_id) {
                case OV9650_ID:
                    DEBUG(" OV9650_ID Sensor is not supported.");
                    return -3;
                    //init_ret = ov9650_init(&sensor);
                    break;
                case OV2640_ID:
                    init_ret = ov2640_init(&sensor);
                    DEBUG(" OV2640_ID Sensor is supported.");
                    break;
                case OV7725_ID:
                    init_ret = ov7725_init(&sensor);
                    DEBUG(" OV7725_ID Sensor is supported.");
                    break;
                default:
...
#endif
int sensor_snapshot_simple(image_t *image)
{
    ...
    if (sensor.pixformat == PIXFORMAT_JPEG) {
        // Sensor has hardware JPEG set max frame size.
        length = MAX_XFER_SIZE;
        addr = (uint32_t) &(MAIN_FB()->pixels[0]);
    } else {
    ...
    // Disable DMA IRQ
    HAL_NVIC_DisableIRQ(&hdma2_stream1);
    ...
    switch (sensor.pixformat) {
        case PIXFORMAT_GRAYSCALE:
        // see DCMI_DMAConvCpltUser, extract Y channel from YUV
            {
                uint8_t *dst = MAIN_FB()->pixels;
                uint8_t *src = MAIN_FB()->pixels;
                
                for(addr = 0; addr < MAIN_FB()->w * MAIN_FB()->h; addr++) {
                    *dst = *src;
                    dst += 1;
                    src += 2;
                }
        ...
        case PIXFORMAT_JPEG:
            // Read the number of data items transferred
            //MAIN_FB()->bpp = (MAX_XFER_SIZE - hdma2_stream1.Instance->NDTR)*4;
            break;
    }
